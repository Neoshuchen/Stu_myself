...
<template>
  <AppShell>
    ...
    <p v-else-if="error && !plan" class="notice error">{{ error }}</p>
    <div v-else class="page-enter">
      <section class="plan-hero" :style="{ '--accent-a': plan.accent_start, '--accent-b': plan.accent_end }">
        <div><span class="card-kicker">{{ plan.subtitle }}</span><h1>{{ plan.title }}</h1><p>{{ plan.summary }}</p>
          ...
          <div class="button-row"><button class="button light" :disabled="busy" @click="enroll">{{ plan.enrolled ? '继续这条路径' : plan.owned ? '加入我的学习路径' : '加入学习计划' }}</button><RouterLink v-if="plan.enrolled" class="button hero-ghost" :to="`/plans/${plan.slug}/community`">进入学习小组</RouterLink><button v-if="plan.enrolled" class="button hero-ghost" :disabled="busy" @click="withdraw">退出这条路线</button><button v-if="plan.review_status === 'approved'" class="button hero-ghost" :disabled="busy" @click="forkPlan">Fork 这条路线</button><RouterLink v-if="plan.editable" class="button hero-ghost" :to="`/plans/${plan.slug}/edit`">编辑路线</RouterLink><button v-if="plan.owned && ['draft', 'rejected'].includes(plan.review_status)" class="button hero-ghost" :disabled="busy" @click="submitReview">提交审核</button><button v-if="plan.owned && !plan.is_published" class="button hero-danger" :disabled="busy" @click="removePlan">删除路线</button></div>
        </div>
      </section>

...
- 课程内容版本：5；系统课程运行时从 MySQL 读取，版本化 JSON 只用于初始化和受控更新。
...
管理员在站点里改过正文的学习日会记录 `content_edited_at`，重新导入时默认跳过这些天并在输出里报告数量，避免把人工修改覆盖掉；确实要用 JSON 目录覆盖它们时加 `--force`（可配合 `--slug` 只处理一条路线）。
